...
fn eval(mal: MalType, env: &mut Env) -> Result<MalType, Error> {
    if !mal.is_collection() {
        return eval_ast(mal, env);
    }

    if mal.is_empty_collection() {
        return Ok(mal);
    }
    ...
    let first_symbol_str = first_symbol.unwrap().get_symbol();
    match first_symbol_str.as_ref() {
        "def!" => {
            let new_list = mal.get_items();
            let symbol_key = new_list[1].get_symbol().clone();
        ...
        "let*" => {
            ...
            for index in (0..binding_list.len()).into_iter().step_by(2) {
                let key = binding_list[index].get_symbol().clone();
