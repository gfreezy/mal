...
fn eval(mal: MalType, env: &mut Env) -> Fallible<MalType> {
    ...
    match new_first_mal {
        MalType::Func(func) => {
            let params = list.into_iter().map(|el| eval(el, env)).collect::<Fallible<Vec<MalType>>>()?;
            func(params)
        }
        MalType::Closure(closure) => {
            ...
            let mut new_env = if let Some(idx) = idx {
                ...
                // drop "&"
                binds.remove(idx);
                let mut exprs: Vec<MalType> = list.into_iter().map(|el| eval(el, env)).collect::<Fallible<Vec<MalType>>>()?;
                ...
                let exprs = list.into_iter().map(|el| eval(el, env)).collect::<Fallible<Vec<MalType>>>()?;
                Env::new(Some(closure.env), binds, exprs)
            };
        ...
        _ => {
            let mut remind: Vec<MalType> = list.into_iter().map(|el| eval(el, env)).collect::<Fallible<Vec<MalType>>>()?;
